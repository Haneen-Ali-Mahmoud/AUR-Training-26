...
class goal_to_goal(Node):
    def __init__(self):
        super().__init__('goal_to_goal')
        ...
        self.service = self.create_service(SetBool,'start_movement',self.service_callback)
    ...
    def service_callback(self, request, response):
        if request.data:
            ...
            response.message = 'Turtle movement started'
            ...
            response.success = True
            response.message = 'Turtle movement stopped'
            self.get_logger().info('Movement STOPPED')

        return response
    
    def control_loop(self):
        if self.current_pose is None or self.goal_reached or not self.movement_active:
            return
